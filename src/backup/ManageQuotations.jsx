...
function ManageQuotations({
  ...
  return (
    ...
    <div className="flex flex-col items-center h-screen bg-gray-50">
      <h2 className="text-4xl font-bold my-4" style={{color:"orange"}}>Manage Quotations</h2>
      {customers.length > 0 ? (
        customers.map((customer, index) => (
          ...
          >
            ...
            <button
              ...
              className="text-blue-500 flex-grow text-left cursor-pointer pl-3 mb-2 hover:underline"
            >
               #{customer.quotationNumber || index + 1}
            </button>
            <h5 style={{ fontSize: '16px', fontWeight: 'bold',textTransform: 'capitalize' , margin:"2px", marginRight:"8px"}}>{customer.customerDetails.customerName || `Customer ${index + 1}`}</h5>
            <button
      ...
      <button type="button" style={{backgroundColor: 'black', color: 'white', padding: '10px 20px', borderRadius: '5px', cursor: 'pointer', marginTop: '20px', background:"orange"}} onClick={() => navigate("/")}>Home</button>
